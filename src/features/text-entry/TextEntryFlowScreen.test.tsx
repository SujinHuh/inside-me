...
function selectFromGlobalSearch(label: string) {
  const search = screen.getByLabelText('감정과 욕구 전체 검색');
  fireEvent.changeText(search, label);
  fireEvent.press(screen.getByLabelText(`${label} 선택`));
...
function advanceToNeeds(story = '합성 테스트를 완료해서 마음이 여러 갈래였다.') {
  ...
  selectFromGlobalSearch('기쁜');
  selectFromGlobalSearch('서운한');
  fireEvent.press(screen.getByLabelText('서운한 강도 4'));
  selectFromGlobalSearch('휴식');
}
...
describe('TextEntryFlowScreen', () => {
  ...
  it('세 마음 방·전역 검색·직접 추가로 C안 탐색을 사용한다', () => {
    render(
      <TextEntryFlowScreen
        dateKey={dateKey('2026-08-23')}
        repository={createRepository()}
    ...
    expect(screen.getByLabelText(/^욕구 방 열기, 110개 표현/)).toBeTruthy();
    expect(screen.getByText('자율성 · 신체·생존 · 사회·정서·상호의존 · 놀이·재미 · 삶의 의미 · 진실성 · 아름다움·평화 · 자기구현')).toBeTruthy();

    fireEvent.press(screen.getByLabelText('충족 감정 방 열기, 70개 표현'));
    expect(screen.getByLabelText(/기쁨과 즐거움 묶음 열기/)).toBeTruthy();
    ...
    fireEvent.press(screen.getByRole('button', { name: '한 단계 뒤로' }));
    fireEvent.press(screen.getByLabelText(/^욕구 방 열기, 110개 표현/));
    expect(screen.getByLabelText(/신체적 안녕과 생존 묶음 열기/)).toBeTruthy();
  ...
  it('AI 화면에서도 감정 선택이나 모름 표시 없이 확인 단계를 우회하지 못한다', () => {
    ...
    expect(screen.getByText('지금 눈이 머무는 마음부터 들어가 보세요.')).toBeTruthy();
