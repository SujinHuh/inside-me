...
현재는 글 기록, 한 장 마음 지도에서 사용자 우선 감정·욕구 탐색, 선택적 보조 후보, 하루 한 기록의 로컬 저장, 달력·날짜 상세, 수정·내보내기·삭제 흐름을 구현했다. 마음 지도에는 충족 감정 70개·미충족 감정 91개·욕구 110개, 총 271개 표현이 들어 있다. 최신 자동 검증은 35개 suite·190개 테스트와 타입·린트·웹 번들을 통과했으며, Android 폰에서 전체 흐름과 긴 지도의 실제 사용성을 직접 확인하는 `IMP-106`은 사용자 실기기 검증 대기다.
...
첫 실제 AI 공급자는 OpenAI API로 확정했다. `IMP-205A`에서 서버 요청·응답 검증, Responses 구조화 출력 요청과 HTTP gateway 경계를 추가했지만, API 키·실제 외부 호출·과금·Expo 조합은 아직 연결하지 않았다. 현재 `AI와 더 살펴보기`는 네트워크를 사용하지 않는 기기 내부 `DeterministicEmotionExplorer`를 계속 호출한다. 남은 원격 구현은 실제 호출 없는 `IMP-205B` 예산 조정자와 `IMP-205C` Expo HTTP/server fake 조합이며, 이후 모델·보존·예산을 결정한다. 실제 OpenAI 합성 계측 `IMP-204C`는 API 키와 별도 승인 대기다. 현재 실행은 Expo Go 개발 컨테이너 안에서 이루어지므로 Mac 개발 서버가 필요하며, 독립 설치형 앱은 핵심 흐름 안정화 뒤 내부 APK 단계에서 만든다.
