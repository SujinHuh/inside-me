...
현재는 글 기록, 한 장 마음 지도에서 사용자 우선 감정·욕구 탐색, 선택적 보조 후보, 하루 한 기록의 로컬 저장, 달력·날짜 상세, 수정·내보내기·삭제 흐름을 구현했다. 마음 지도에는 충족 감정 70개·미충족 감정 91개·욕구 110개, 총 271개 표현이 들어 있다. 최신 자동 검증은 36개 suite·205개 테스트와 타입·린트·웹 번들을 통과했으며, Android 폰에서 전체 흐름과 긴 지도의 실제 사용성을 직접 확인하는 `IMP-106`은 사용자 실기기 검증 대기다.

DEC-057에 따라 2026-08-30까지의 우선 완료선은 `글로 기록하기 + 선택적 실제 OpenAI 보조 + 로컬 저장·달력 복원`의 Android Expo Go 도그푸딩 MVP다. 자유 음성 기록과 AI 음성 핑퐁은 취소하지 않고 텍스트 MVP 안정화 후의 후속 단계로 둔다.

첫 실제 AI 공급자는 OpenAI API로 확정했다. `IMP-205A`에서 서버 요청·응답 검증, Responses 구조화 출력 요청과 HTTP gateway 경계를 추가했고, `IMP-205B`에서 실제 호출 없는 예산 예약·usage 정산·취소·실패 복구 조정자를 구현해 자동 검증과 독립 재검수 오류 0개를 확인했다. API 키·실제 외부 호출·과금·Expo 조합은 아직 연결하지 않았다. 현재 `AI와 더 살펴보기`는 네트워크를 사용하지 않는 기기 내부 `DeterministicEmotionExplorer`를 계속 호출한다. 다음 원격 구현은 `IMP-205C` Expo HTTP/server fake 조합이며, 이후 모델·보존·예산을 결정한다. 실제 OpenAI 합성 계측 `IMP-204C`는 API 키와 별도 승인 대기다. 현재 실행은 Expo Go 개발 컨테이너 안에서 이루어지므로 Mac 개발 서버가 필요하며, 독립 설치형 앱은 핵심 흐름 안정화 뒤 내부 APK 단계에서 만든다.
...
4. 사용자가 충족·미충족 감정과 욕구 8개 영역이 함께 보이는 한 장 마음 지도를 직접 둘러보고 자신에게 가까운 표현을 먼저 고른다.
...
프로젝트 구조는 별도 `frontend/`, `backend/` 프로젝트를 먼저 만들지 않고 Expo 클라이언트와 모듈형 모놀리스의 `server/ai/` 경계를 한 저장소에서 구분한다. 서버 코드는 현재 Expo 런타임에 조합되지 않아 비밀 키나 외부 호출이 클라이언트 번들에 들어가지 않는다.
...
감정·욕구 자기 탐색 화면을 만들기 전에는 [사용자 제공 어휘 이미지 참고 노트](docs/references/emotion-needs-vocabulary-images.md)와 [전체 카탈로그 대조](docs/references/emotion-needs-vocabulary-catalog.md)를 확인한다.
