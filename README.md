...
현재는 **MVP 0단계 Expo 기반 자동 검증 완료, IMP-002R 독립 검수 중** 상태다. Node 24 LTS, Expo SDK 54와 Expo Router 기반의 실행 가능한 최소 앱이 있다. 독립 검수에서 확인된 오류를 모두 해소한 뒤에만 공통 계약·감정·욕구 어휘·가짜 구현·디자인 토큰을 고정하는 `IMP-003`으로 넘어간다. Android 실기기에서 Expo Go QR 실행은 아직 확인하지 않았다.
